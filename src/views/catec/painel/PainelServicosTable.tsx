...
import type { CatecServicoPainelItem, CatecServicoStatus } from '@/types/catec/servicoTypes'
...
type Props = {
  ...
  statusFiltro: CatecServicoStatus | null
  onStatusFiltroChange: (status: CatecServicoStatus | null) => void
}

const globalFilterFn: FilterFn<CatecServicoPainelItem> = (row, _columnId, filterValue) => {
...
const columnHelper = createColumnHelper<CatecServicoPainelItem>()

const PainelServicosTable = ({ servicos, statusFiltro, onStatusFiltroChange }: Props) => {
  ...
  }, [servicos, statusSelect, clienteFiltro, faixaPrazo])

  const columns = useMemo<ColumnDef<CatecServicoPainelItem, any>[]>(
    () => [
      columnHelper.accessor('titulo', {
        header: 'Servico',
        cell: ({ row }) => (
          <Link href={`/catec/servicos/${row.original.id}`} className='font-medium text-textPrimary hover:text-primary'>
            {row.original.titulo}
          </Link>
        )
      }),
      ...
      columnHelper.display({
        id: 'status',
        header: 'Status',
        cell: ({ row }) => <ServicoStatusBadge status={row.original.status} />
      }),
      columnHelper.display({
        id: 'previsao',
        header: 'Prazo ativo',
  ...
  return (
    <Card>
      <CardHeader title='Todos os servicos' subheader='Filtre por status, cliente ou faixa de prazo' />
      <div className='flex flex-col gap-4 p-6 pt-0'>
        <div className='flex flex-col lg:flex-row gap-4'>
          <CustomTextField
            ...
            onChange={e => {
              ...
              onStatusFiltroChange(value ? (value as CatecServicoStatus) : null)
            }}
          ...
          >
            ...
            {ORDEM_STATUS_SERVICO.map(status => (
              <MenuItem key={status} value={status}>
                {STATUS_SERVICO_ROTULO_BADGE[status]}
              </MenuItem>
            ))}
          </CustomTextField>
          <CustomTextField
        ...
        <div className='flex justify-between flex-col items-start md:flex-row md:items-center gap-4'>
          ...
          <DebouncedInput
            ...
            placeholder='Buscar servico ou cliente'
            ...
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                <td colSpan={table.getVisibleFlatColumns().length} className='text-center'>
                  <Typography className='p-6' color='text.secondary'>
                    Nenhum servico corresponde aos filtros.
                  </Typography>
                </td>
              </tr>
            ) : (
              table.getRowModel().rows.map(row => (
                <tr
                  ...
                  onClick={() => router.push(`/catec/servicos/${row.original.id}`)}
                >
...
export default PainelServicosTable
