...
import type { CatecServico, CatecServicoCreateInput } from '@/types/catec/servicoTypes'
...
import ServicoAddDrawer from './ServicoAddDrawer'
import ServicoTableFilters from './ServicoTableFilters'

type ServicoRow = CatecServico & { action?: string }

const globalFilterFn: FilterFn<ServicoRow> = (row, _columnId, filterValue) => {
...
const columnHelper = createColumnHelper<ServicoRow>()

type Props = {
  lista: CatecServico[]
  clientes: CatecCliente[]
  onAdd: (input: CatecServicoCreateInput) => Promise<CatecServico>
}

const ServicoListTable = ({ lista, clientes, onAdd }: Props) => {
  ...
  const columns = useMemo<ColumnDef<ServicoRow, any>[]>(
    () => [
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <IconButton aria-label='Abrir servico'>
              <Link href={`/catec/servicos/${row.original.id}`} className='flex'>
  ...
  return (
    <>
      <Card>
        ...
        <ServicoTableFilters setData={setFilteredData} tableData={lista} />
                ...
                <tr>
                  <td colSpan={table.getVisibleFlatColumns().length} className='text-center'>
                    <Typography className='p-6' color='text.secondary'>
                      Não há servicos que correspondam aos filtros.
                    </Typography>
                  </td>
                </tr>
              ) : (
      ...
      <ServicoAddDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} clientes={clientes} onAdd={onAdd} />
    </>
  )
}

export default ServicoListTable
