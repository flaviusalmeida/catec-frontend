...
import {
  ...
  listarAtividadesPorServicoCatec,
  obterAtividadeCatec
...
import ServicoAtividadeLinha from './ServicoAtividadeLinha'
import styles from './servicoAtividades.module.css'
...
const STATUS_LEITURA: CatecServicoStatus[] = [
  'AGUARDANDO_EXECUCAO',
  'EM_EXECUCAO',
  'FINALIZADO',
  'CANCELADO'
...
const STATUS_CRIACAO: CatecServicoStatus[] = ['AGUARDANDO_EXECUCAO', 'EM_EXECUCAO']
...
const ServicoTabAtividades = ({ servico }: Props) => {
  ...
  const permiteLeitura = STATUS_LEITURA.includes(servico.status)
  const permiteCriacao = STATUS_CRIACAO.includes(servico.status)
  const boardHref = `/catec/atividades?servicoId=${servico.id}`
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const data = await listarAtividadesPorServicoCatec(servico.id)

      setLista(data)
      setColapsadas({})
      void carregarStore({ servicoId: servico.id })
    } catch (err) {
      setLista([])
  ...
  }, [carregarStore, permiteLeitura, servico.id])
  ...
  const handleCriar = async (e: FormEvent) => {
    ...
    try {
      await criarAtividadeRaizCatec(servico.id, { titulo: t })
  ...
  if (!permiteLeitura) {
    return (
      <Alert severity='info' variant='outlined'>
        Atividades ficam disponíveis quando o servico estiver em execução (aguardando execução, em execução) ou
    ...
    <>
      <Grid container spacing={4}>
        <Grid size={{ xs: 12 }} className='flex flex-wrap items-center justify-between gap-3'>
          <Typography variant='h5'>Atividades do servico</Typography>
        ...
        {carregando ? (
          ...
          <Grid size={{ xs: 12 }}>
            <Alert severity='info' variant='outlined'>
              Nenhuma atividade cadastrada neste servico.
            </Alert>
          </Grid>
        ) : (
          <Grid size={{ xs: 12 }}>
            <CanPermission
              anyOf={[PermissaoCodigo.TELA_ATIVIDADES, PermissaoCodigo.TELA_SERVICO_DETALHE]}
              fallback={
...
export default ServicoTabAtividades
