...
const ServicoTabPropostas = ({ servico, fluxo }: Props) => {
  ...
  const servicoTemCliente = servico.clienteId != null
  ...
  const mostrarUpload = servicoTemCliente && (podeUploadExistente || podeIniciarProposta)
  ...
  if (!servicoTemCliente) {
    return (
      <ServicoStateCard
        titulo='Nenhuma proposta cadastrada.'
        descricao='Associe um cliente ao servico.'
      />
    )
  }

  if (data.propostas.length === 0 && !mostrarUpload) {
    return <ServicoStateCard titulo='Nenhuma proposta cadastrada.' />
  }

  const previewPropostaSubtitulo = propostaAtual
    ? `${servico.titulo} · v${propostaAtual.versao}`
    : servico.titulo
    ...
    <Grid container spacing={6}>
              ...
              {versoesAnteriores.map(proposta => {
                ...
                return (
                  <ServicoFileRow
                    ...
                    documentoId={doc.id}
                    previewTitulo='Proposta comercial'
                    previewSubtitulo={`${servico.titulo} · v${proposta.versao}`}
                    onDownload={() =>
                    void downloadDocumentoCatec(doc.id, doc.nomeOriginal).catch(err =>
                      toast.error(err instanceof Error ? err.message : 'Download falhou.')
                    )
      ...
      <Dialog open={dialogInteracaoCliente != null} onClose={fecharDialogInteracaoCliente} fullWidth maxWidth='sm'>
        ...
        <DialogContent className='flex flex-col gap-4 pbs-2'>
          {propostaAtual ? (
            <Typography variant='body2' color='text.secondary'>
              {servico.titulo} · v{propostaAtual.versao} · {STATUS_PROPOSTA_ROTULO[propostaAtual.status]}
            </Typography>
          ) : null}
          <CustomTextField
            fullWidth
...
export default ServicoTabPropostas
